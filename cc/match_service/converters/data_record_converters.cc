...
absl::Status ToBackend(const api::v1::DataRecord& in,
                       backend::DataRecord& out) {
  out.Clear();
  for (const auto& match_key : in.match_keys()) {
    RETURN_IF_ERROR(ToBackend(match_key, *out.add_match_keys()));
  }
...
absl::Status ToApi(const backend::DataRecord& in, api::v1::DataRecord& out) {
  out.Clear();
  for (const auto& match_key : in.match_keys()) {
    RETURN_IF_ERROR(ToApi(match_key, *out.add_match_keys()));
  }
  if (in.has_encryption_key()) {
