...
class HybridDecryptCryptoKeyTest : public testing::Test {
 protected:
  HybridDecryptCryptoKeyTest()
      : mock_hybrid_decrypt_(std::make_shared<MockHybridDecrypt>()),
        crypto_key_(mock_hybrid_decrypt_) {}

  std::shared_ptr<MockHybridDecrypt> mock_hybrid_decrypt_;
  HybridDecryptCryptoKey crypto_key_;
};

TEST_F(HybridDecryptCryptoKeyTest, EncryptFails) {
  EXPECT_THAT(crypto_key_.Encrypt(kPlaintext),
              absl_testing::StatusIs(absl::StatusCode::kUnimplemented));
}

TEST_F(HybridDecryptCryptoKeyTest, DecryptIsSuccessful) {
...
TEST_F(HybridDecryptCryptoKeyTest, DecryptWithErrorReturnsFailure) {
  EXPECT_CALL(*mock_hybrid_decrypt_, Decrypt(kCiphertext, ""))
