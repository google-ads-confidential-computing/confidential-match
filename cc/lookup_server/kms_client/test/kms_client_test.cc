...
constexpr absl::string_view kKmsResourceName =
    ...
    "cryptoKeys/test-key";
constexpr absl::string_view kKmsResourceNameWithGcpResourcePrefix =
...
// Helper to simulate a successful response from the CPIO KMS client while
// validating the request proto.
ExecutionResult MockDecryptSuccessfulValidatingRequest(
    const google::cmrt::sdk::kms_service::v1::DecryptRequest& expected_request,
    absl::string_view response,
    AsyncContext<google::cmrt::sdk::kms_service::v1::DecryptRequest,
                 google::cmrt::sdk::kms_service::v1::DecryptResponse>&
        decrypt_context) noexcept {
...
TEST_F(KmsClientTest, DecryptWithGcpKmsResourcePrefixReturnsSuccess) {
  AsyncContext<DecryptRequest, std::string> context;
  context.request = std::make_shared<DecryptRequest>();
  context.request->set_key_resource_name(kKmsResourceNameWithGcpResourcePrefix);
  context.request->set_gcp_wip_provider(kWipProvider);
  context.request->set_account_identity(kVerifiedIdentity);
  context.request->set_ciphertext(kCiphertext);

  DecryptRequest expected_request(*context.request);
  expected_request.set_key_resource_name(kKmsResourceName);
  EXPECT_CALL(*mock_cpio_kms_client_, Decrypt)
      .WillOnce(std::bind(MockDecryptSuccessfulValidatingRequest,
                          expected_request, kPlaintext, _1));
  ...
  context.callback =
      [&is_complete](AsyncContext<DecryptRequest, std::string>& ctx) {
        EXPECT_SUCCESS(ctx.result);
        EXPECT_EQ(*ctx.response, kPlaintext);
        EXPECT_EQ(ctx.request->key_resource_name(),
                  kKmsResourceNameWithGcpResourcePrefix);
        is_complete = true;
      };
