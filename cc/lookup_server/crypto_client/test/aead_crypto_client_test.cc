 ...
 protected:
  AeadCryptoClientTest()
      : mock_kms_client_(std::make_shared<MockKmsClient>()),
        crypto_client_(std::make_unique<AeadCryptoClient>(mock_kms_client_)) {}
...
EncryptionKeyInfo GetEncryptionKeyInfo() {
  ...
  wrapped_key_info->set_kek_kms_resource_id(kKmsResourceName);
...
TEST_F(AeadCryptoClientTest, GetCryptoKeySuccess) {
  AsyncContext<EncryptionKeyInfo, CryptoKeyInterface> context;
  context.request = std::make_shared<EncryptionKeyInfo>(GetEncryptionKeyInfo());
  std::string decoded_dek;
  EXPECT_TRUE(absl::Base64Unescape(kTestDek, &decoded_dek));
  EXPECT_CALL(*mock_kms_client_, DecryptAsync)
      .WillOnce(std::bind(MockKmsDecryptWithResponse, decoded_dek, _1));
